...
public partial class ExpensesViewModel(ApiClient apiClient) : ObservableObject
{
    [ObservableProperty] private ObservableCollection<TransactionDto> expenses = [];
    ...
    private async Task LoadAsync()
    {
        try
        {
            IsBusy = true;
            ErrorMessage = null;
            var result = await apiClient.GetTransactionsAsync("expense");
            Expenses = new ObservableCollection<TransactionDto>(result);
        }
        catch (Exception ex)
        {
